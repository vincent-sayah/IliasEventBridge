 ...
 */
class ilIliasEventBridgeCourseResourceResolver
{
    ...
    {
        $this->trackingRepository = $trackingRepository instanceof ilIliasEventBridgeCourseTrackingRepository
            ? $trackingRepository
            : null;
    ...
    public function resolveCourse(int $courseRefId): array
    {
        ...
        $courseObjId = $this->lookupTypeByRefId($courseRefId) === 'crs'
            ? $this->lookupObjectId($courseRefId)
            : 0;
    ...
    private function getResourceConfigsByRefId(int $courseRefId): array
    {
        ...
        if (!$repository instanceof ilIliasEventBridgeCourseTrackingRepository || !$repository->resourceTableExists()) {
            return [];
        }
    ...
    {
        $repository = $this->getTrackingRepository();
        if (!$repository instanceof ilIliasEventBridgeCourseTrackingRepository || !$repository->courseTableExists()) {
            return [];
        }
    ...
    {
        if ($this->trackingRepository instanceof ilIliasEventBridgeCourseTrackingRepository) {
            return $this->trackingRepository;
        }

        if (!class_exists('ilIliasEventBridgeCourseTrackingRepository')) {
            return null;
        }

        try {
            $this->trackingRepository = new ilIliasEventBridgeCourseTrackingRepository();
        } catch (Throwable $ignored) {
