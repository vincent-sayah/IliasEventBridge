 ...
 * - logs denied statements in evnt_evhk_ileb_dlog only when deny diagnostics are enabled.
 */
class ilIliasEventBridgeEventRouter
{
    ...
    /** @var ilIliasEventBridgePlugin */
    private $plugin;

    /** @var ilIliasEventBridgeStatementFactory|null */
    private $statementFactory;

    /** @var ilIliasEventBridgeOutboxRepository|null */
    private $outboxRepository;

    /** @var ilIliasEventBridgeCourseContextResolver|null */
    private $courseContextResolver;

    /** @var ilIliasEventBridgeCourseTrackingRepository|null */
    private $courseTrackingRepository;

    /** @var ilIliasEventBridgeDenyLogRepository|null */
    private $denyLogRepository;

    public function __construct(
        ilIliasEventBridgeConfig $config,
        ilIliasEventBridgeEventDebugRepository $repository,
        ilIliasEventBridgePlugin $plugin,
        ?ilIliasEventBridgeStatementFactory $statementFactory = null,
        ?ilIliasEventBridgeOutboxRepository $outboxRepository = null,
        ?ilIliasEventBridgeCourseContextResolver $courseContextResolver = null,
        ?ilIliasEventBridgeCourseTrackingRepository $courseTrackingRepository = null,
        ?ilIliasEventBridgeDenyLogRepository $denyLogRepository = null
    ) {
        $this->config = $config;
    ...
    public function handle(string $component, string $event, array $params): void
    {
        ...
        $eventLogId = $this->config->isDebugEnabled()
        ...
        if (!$courseContext['is_in_course']) {
            $this->logDeniedTrace('not_in_course', $record, 'evnt_evhk_ileb_log', $eventLogId);
            return;
        }
        ...
        if ($denialReason !== '') {
            $this->logDeniedTrace($denialReason, $record, 'evnt_evhk_ileb_log', $eventLogId);
            return;
        }
        ...
        if ($enqueued === 0) {
            $this->logDeniedTrace('unsupported_object_type', $record, 'evnt_evhk_ileb_log', $eventLogId);
        }
    }
    ...
    private function getCourseContextResolver(): ilIliasEventBridgeCourseContextResolver
    {
        if ($this->courseContextResolver === null) {
            $this->courseContextResolver = new ilIliasEventBridgeCourseContextResolver();
        }

        return $this->courseContextResolver;
    }

    private function getCourseTrackingRepository(): ilIliasEventBridgeCourseTrackingRepository
    {
        if ($this->courseTrackingRepository === null) {
            $this->courseTrackingRepository = new ilIliasEventBridgeCourseTrackingRepository();
        }

        return $this->courseTrackingRepository;
    }

    private function getDenyLogRepository(): ilIliasEventBridgeDenyLogRepository
    {
        if ($this->denyLogRepository === null) {
            $this->denyLogRepository = new ilIliasEventBridgeDenyLogRepository();
        }
    ...
    private function trackingDenialReason(int $courseRefId, int $refId): string
    {
        ...
        // The course access itself is controlled by the course switch. It is
        // not a child resource and therefore has no row in the resource table.
        if ($refId === $courseRefId) {
            return '';
        }
