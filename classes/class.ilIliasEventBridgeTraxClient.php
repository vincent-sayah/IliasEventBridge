...
require_once __DIR__ . '/class.ilIliasEventBridgeHttpResult.php';
 ...
 */
class ilIliasEventBridgeTraxClient
{
    ...
    public function testConnection(): ilIliasEventBridgeHttpResult
    {
        if (!$this->config->isTraxConfigured()) {
            return new ilIliasEventBridgeHttpResult(false, 0, '', 'Configuration TRAX incomplète.');
        }
     ...
     */
    public function sendStatements(array $statements): ilIliasEventBridgeHttpResult
    {
        if (!$this->config->isTraxConfigured()) {
            return new ilIliasEventBridgeHttpResult(false, 0, '', 'Configuration TRAX incomplète.');
        }
        ...
        if (count($statements) === 0) {
            return new ilIliasEventBridgeHttpResult(true, 0, '', 'Aucun statement à envoyer.');
        }

        $payload = json_encode($statements, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        if (!is_string($payload)) {
            return new ilIliasEventBridgeHttpResult(false, 0, '', 'Impossible d’encoder le batch xAPI en JSON.');
        }

        return $this->request('POST', $this->config->getStatementsEndpoint(), $payload);
    }

    private function request(string $method, string $url, ?string $payload): ilIliasEventBridgeHttpResult
    {
    ...
    private function requestWithCurl(string $method, string $url, ?string $payload): ilIliasEventBridgeHttpResult
    {
        $ch = curl_init($url);
        if ($ch === false) {
            return new ilIliasEventBridgeHttpResult(false, 0, '', 'curl_init a échoué.');
        }
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $this->config->isTlsVerificationEnabled());
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, $this->config->isTlsVerificationEnabled() ? 2 : 0);

        $caBundle = $this->config->getCaBundlePath();
        if ($caBundle !== '') {
        ...
        if ($body === false) {
            return new ilIliasEventBridgeHttpResult(false, $status, '', $error !== '' ? $error : 'Erreur cURL inconnue.');
        }

        $success = $status >= 200 && $status < 300;
        return new ilIliasEventBridgeHttpResult($success, $status, (string) $body, $success ? '' : $this->extractError((string) $body));
    }

    private function requestWithStream(string $method, string $url, ?string $payload): ilIliasEventBridgeHttpResult
    {
        $headers = $this->headers();
        ...
        $context = [
            ...
            'ssl' => [
                'verify_peer' => $this->config->isTlsVerificationEnabled(),
                'verify_peer_name' => $this->config->isTlsVerificationEnabled(),
            ],
        ];

        $caBundle = $this->config->getCaBundlePath();
        if ($caBundle !== '') {
            $context['ssl']['cafile'] = $caBundle;
        }
        ...
        if ($body === false) {
            ...
            return new ilIliasEventBridgeHttpResult(false, $status, '', is_array($error) ? (string) ($error['message'] ?? '') : 'Erreur HTTP stream.');
        }
