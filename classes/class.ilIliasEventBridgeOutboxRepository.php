 ...
 */
class ilIliasEventBridgeOutboxRepository
{
    public const TABLE_NAME = 'evnt_evhk_ileb_out';
    ...
    private function detectEventType(array $eventRecord): string
    {
        ...
        if ($component === 'components/ILIAS/ReadEvent' && $event === 'access' && $type === 'mcst' && strpos($uri, 'ileb_mcst_event=') !== false) {
        ...
        if (($component === 'Services/Tracking' || $component === 'components/ILIAS/Tracking') && $event === 'updateStatus') {
    ...
    {
        return in_array($type, ['crs', 'file', 'blog', 'webr', 'mcst', 'frm', 'wiki', 'htlm', 'lm', 'sahs', 'exc'], true);
    }
}
